...
def _chart_funding_gauge(pct_funded: float) -> io.BytesIO:
    ...
    color = (
        "#C0392B" if pct_funded < 30 else ("#D97706" if pct_funded < 70 else "#1E8449")
    )
    ...
    ax.text(
        1.22 * np.cos(bench_rad),
        1.22 * np.sin(bench_rad) + 0.04,
        "70%\nbenchmark",
        fontsize=7.5,
        color="#2C3E50",
        ha="center",
    ...
    ax.text(0, -0.30, "funded", ha="center", va="center", fontsize=10, color="#666666")

    ax.set_title(
        "Percent Funded", fontsize=12, fontweight="bold", color="#222222", pad=4
    )
...
def _chart_balance_projection(plan_rows: list[dict[str, Any]]) -> io.BytesIO:
    ...
    ax.set_title(
        f"{len(years)}-Year Reserve Balance Projection",
        fontsize=11,
        fontweight="bold",
        color="#222222",
        pad=10,
...
def _chart_expenditures(plan_rows: list[dict[str, Any]]) -> io.BytesIO:
    ...
    ax.set_title(
        "Planned Capital Expenditures by Year",
        fontsize=11,
        fontweight="bold",
        color="#222222",
        pad=10,
...
def generate_reserve_study_docx(
    ...
    opening_balance = (
        _q2(override) if override is not None else repo.get_reserve_fund_balance()
    )
    ...
    status_label = (
        ...
        if pct_funded < 30
        else "BELOW INDUSTRY BENCHMARK" if pct_funded < 70 else "ADEQUATELY FUNDED"
    )
    status_color = (
        "C0392B" if pct_funded < 30 else ("D97706" if pct_funded < 70 else "1E8449")
    )
    ...
    for i, (label, value) in enumerate(metrics):
        row = tbl.rows[i + 1]
        _cell_text(
            row.cells[0], label, bold=False, shade=_ALT_BG if i % 2 == 0 else None
        )
        _cell_text(
            row.cells[1],
            value,
    ...
    for group_name, group_assets in groups.items():
        ...
        for i, a in enumerate(group_assets):
            ...
            future = (
                _q2(cost * (1 + inflation) ** years_out) if cost > 0 else Decimal("0")
            )
            ...
            _cell_text(
                row.cells[5],
                _money(cost) if cost else "$0",
                shade=shade,
                align=WD_ALIGN_PARAGRAPH.RIGHT,
            )
            _cell_text(
                row.cells[6],
                f"{float(a['annual_inflation'])*100:.0f}%",
                shade=shade,
                align=WD_ALIGN_PARAGRAPH.CENTER,
            )
            _cell_text(
                row.cells[7],
                _money(future) if future else "$0",
                shade=shade,
                align=WD_ALIGN_PARAGRAPH.RIGHT,
            )
    ...
    for i, (rating, desc) in enumerate(
        ...
        _cell_text(
            key_tbl.rows[i + 1].cells[0],
            rating,
            shade=_ALT_BG if i % 2 == 0 else None,
            color=condition_pill.get(rating, "000000"),
            bold=True,
        )
        _cell_text(
            key_tbl.rows[i + 1].cells[1], desc, shade=_ALT_BG if i % 2 == 0 else None
        )
    ...
    for i, pr in enumerate(plan_rows):
        ...
        _cell_text(
            row.cells[1],
            _money(pr["beginning_balance"]),
            align=WD_ALIGN_PARAGRAPH.RIGHT,
            shade=shade,
            color="C0392B" if pr["beginning_balance"] < 0 else None,
        )
        _cell_text(
            row.cells[2],
            _money(pr["contribution"]),
            align=WD_ALIGN_PARAGRAPH.RIGHT,
            shade=shade,
        )
    ...
    for i, s in enumerate(scenarios):
        ...
        _cell_text(
            row.cells[2], _money(cost), align=WD_ALIGN_PARAGRAPH.RIGHT, shade=shade
        )
        _cell_text(
            row.cells[3],
            _money(per_lot),
    ...
    assump_data = [
        ...
        (
            "Projection Period",
            f"{proj_years} years ({study_year}–{study_year + proj_years - 1})",
        ),
    ...
    for i, (label, value) in enumerate(assump_data):
        ...
        _cell_text(
            tbl.rows[i + 1].cells[1], value, shade=shade, align=WD_ALIGN_PARAGRAPH.RIGHT
        )
    _set_col_widths(tbl, [3.8, 1.8])
