    ...
    for tab in tabs:
        sections = [
            ...
            for r in conn.execute(
                ...
                "WHERE tab_id=? AND is_active=1 ORDER BY sort_order, id",
                (tab["id"],),
            ).fetchall()
        ]
        for sec in sections:
...
class WorkflowAdminService:
    ...
    def add_card(
        ...
        max_sort = (
            self._conn.execute(
                "SELECT MAX(sort_order) FROM workflow_cards WHERE section_id=?",
                (section_id,),
            ).fetchone()[0]
            or 0
        ) + 10
    ...
    def delete_card(self, card_id: int) -> None:
        self._conn.execute(
            "DELETE FROM workflow_cards WHERE id=? AND is_system=0", (card_id,)
        )
        self._conn.commit()
    ...
    def reorder_card(self, card_id: int, direction: str) -> None:
        ...
        if neighbor:
            self._conn.execute(
                "UPDATE workflow_cards SET sort_order=? WHERE id=?",
                (neighbor["sort_order"], card_id),
            )
            self._conn.execute(
                "UPDATE workflow_cards SET sort_order=? WHERE id=?",
                (cur_sort, neighbor["id"]),
            )
            self._conn.commit()
    ...
    def add_section(self, tab_id: int, label: str, tip_text: str = "") -> int:
        max_sort = (
            self._conn.execute(
                "SELECT MAX(sort_order) FROM workflow_sections WHERE tab_id=?",
                (tab_id,),
            ).fetchone()[0]
            or 0
        ) + 10
    ...
    def add_tab(self, tab_key: str, icon: str, label: str, description: str) -> int:
        ...
        existing = {
            r[0]
            for r in self._conn.execute("SELECT tab_key FROM workflow_tabs").fetchall()
        }
        base = tab_key
        n = 2
        while tab_key in existing:
            ...
            n += 1
        max_sort = (
            self._conn.execute("SELECT MAX(sort_order) FROM workflow_tabs").fetchone()[
                0
            ]
            or 0
        ) + 10
