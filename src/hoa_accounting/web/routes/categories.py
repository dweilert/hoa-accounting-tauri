...
def make_categories_blueprint(ctx: RouteContext) -> Blueprint:
    ...
    def list_categories() -> ResponseReturnValue:
        ...
        resp = ctx.open_pages(CategoriesPages).render_list(
    ...
    def new_category_form() -> ResponseReturnValue:
        theme = str(org_context.get("theme", "warm"))
        resp = ctx.open_pages(CategoriesPages).render_add_form(
    ...
    def edit_category_form(category_id: int) -> ResponseReturnValue:
        theme = str(org_context.get("theme", "warm"))
        resp = ctx.open_pages(CategoriesPages).render_edit_form(
            category_id, org=org_context, theme=theme
        )
        return Response(
            resp.body_html, status=resp.status_code, mimetype="text/html; charset=utf-8"
        )
    ...
    def view_category_ledger(category_id: int) -> ResponseReturnValue:
        theme = str(org_context.get("theme", "warm"))
        resp = ctx.open_pages(CategoriesPages).render_ledger(
            category_id, org=org_context, theme=theme
        )
        return Response(
            resp.body_html, status=resp.status_code, mimetype="text/html; charset=utf-8"
