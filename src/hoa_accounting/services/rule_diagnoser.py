...
def diagnose(rule: dict[str, Any], txn: Any) -> MatchReport:
    ...
    if type_pat:
        if type_pat.lower() in txn_type.lower():
            ...
            if type_pat.upper() == "DEP" and txn_type.upper() == "CREDIT":
                sug = ("OFX 1.x typically writes deposits as 'CREDIT', "
                ...
                sug = (f"Set Type to {txn_type!r}, or clear it to match "
    ...
    would = is_active and all_ok and any(
        # at least ONE criterion must be set; an all-blank rule is a wildcard
        rule.get(k) for k in ("description_contains", "match_memo",
                              "match_type", "match_amount", "bank_account_id")
    )
    # If no criteria are set the rule is a wildcard that fires on every row
    # for its account. Reflect that.
    if not any(rule.get(k) for k in ("description_contains", "match_memo",
                                     "match_type", "match_amount")):
