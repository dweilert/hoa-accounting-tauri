...
def _post_assessment(conn, ids, *, amount: str):
    return _factory(conn).assessment_service().post_assessment(
        entry_date="2026-01-01", lot_id=ids.lot1_id, owner_id=ids.owner1_id,
        amount=amount, description="dues", category_id=ids.cat_dues_id,
        charge_type="DUES", due_date="2026-01-15",
    ).assessment_id


def test_post_payment_no_application_creates_row():
    conn, ids = build_seeded_conn()
    res = _factory(conn).payment_service().post_payment(
        entry_date="2026-01-20",
        owner_id=ids.owner1_id, amount="125.00",
    ...
    row = conn.execute(
        ...
        "FROM payments WHERE id = ?", (res.payment_id,),
    ).fetchone()
...
def test_post_payment_applies_to_assessment_marks_paid():
    ...
    res = _factory(conn).payment_service().post_payment(
        entry_date="2026-01-20",
        owner_id=ids.owner1_id, amount="100.00",
        bank_account_id=ids.bank_op_id,
        payment_method="CHECK",
...
def test_post_payment_partial_application_marks_partial():
    ...
    _factory(conn).payment_service().post_payment(
        entry_date="2026-01-20", owner_id=ids.owner1_id, amount="40.00",
        bank_account_id=ids.bank_op_id, payment_method="CHECK",
        receipt_number="RCT-003", description="x",
        apply_to_assessment_ids=[a_id],
    )
    status = conn.execute(
        "SELECT status FROM assessments WHERE id = ?", (a_id,)
...
def test_post_payment_rejects_zero_amount():
    ...
    with pytest.raises(ValidationError):
        _factory(conn).payment_service().post_payment(
            entry_date="2026-01-20", owner_id=ids.owner1_id, amount="0",
            bank_account_id=ids.bank_op_id, payment_method="CHECK",
...
def test_post_payment_rejects_unknown_owner():
    ...
    with pytest.raises(NotFoundError):
        _factory(conn).payment_service().post_payment(
            entry_date="2026-01-20", owner_id=9999, amount="50",
            bank_account_id=ids.bank_op_id, payment_method="CHECK",
...
def test_post_payment_rejects_unknown_bank():
    ...
    with pytest.raises(NotFoundError):
        _factory(conn).payment_service().post_payment(
            entry_date="2026-01-20", owner_id=ids.owner1_id, amount="50",
            bank_account_id=9999, payment_method="CHECK",
